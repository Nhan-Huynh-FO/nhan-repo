...
class MobileController extends Zend_Controller_Action
{
    ...
    public function othervideoAction()
    {
        ...
        //get all article by rule 2
        $arrVideo = $newsModel->getListArticleIdsByRule2($arrParam);

        // Assign to view
        $this->view->assign(array(
            'arrData' => $arrVideo['data']
        ));

        $response = array(
            'error' => 0,
            'offset' => $offset + LIMIT_LIST_VIDEO,
            'total' => count($arrVideo['data']),
            'html' => '',
        );

        if (!empty($arrVideo['data']))
        {
            //set id
            $this->view->objArticle->setIdBasic($arrVideo['data']);
            //Render view
            $response['html'] = $this->view->render('mobile/othervideo.phtml');
        }
        else
        {
            $response['error'] = 1;
