...
class DetailController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        if (!empty($viewApp) && $viewApp == 'app')
        {
            ...
            $this->view->assign(array(
                ...
                'isDetail' => 1,
                'ogType' => 'website',
                'ogTitle' => $arrArticleDetail['title_format'] . ' - VnExpress SEA Games',
        ...
        $this->view->assign(array(
            ...
            'ogType' => 'article',
            'ogTitle' => $arrArticleDetail['title_format'] . ' - VnExpress SEA Games',
            'ogUrl' => $this->view->ShareurlSeagame($arrArticleDetail['share_url']),
            'ogImage' => !empty($arrArticleDetail['thumbnail_url']) ? $this->view->ImageurlArticle($arrArticleDetail, 'size2') : 'http://m.f9.img.vnexpress.net/2014/01/10/fa68f81afe10b727852361b1894ccc74.jpg',
            'ogDescription' => trim(strip_tags(html_entity_decode($arrArticleDetail['lead'], ENT_COMPAT, 'UTF-8')))
        ));
        // SEO
    ...
    public function photoAction()
    {
        ...
        $this->view->assign(array(
            ...
            'isArticlePhoto' => true,
            'ogType' => 'website',
            'ogTitle' => $arrData['title_format'] . ' - VnExpress SEA Games',
            'ogUrl' => $this->view->ShareurlSeagame($arrData['share_url']),
        ...
        $this->view->headMeta()->setName('description', trim(strip_tags(html_entity_decode($arrData['lead'], ENT_COMPAT, 'UTF-8'))) . ' - VnExpress SEA Games')
            ->setName('keywords', $arrData['title_format'] . ' - VnExpress SEA Games');
    }
    ...
    public function tuongThuatAction()
    {
        ...
        //get detail article
        $arrArticleDetail = $this->view->objArticle->getArticleFull($intArticleId);
        $arrArticleDetail['share_url'] = $this->view->ShareurlSeagame($arrArticleDetail['share_url']);
        $arrArticleDetail['share_url'] = preg_replace('#/tin-tuc/sea-games/#', '/tuong-thuat/', $arrArticleDetail['share_url']);

        //check link info
        $strLink = $this->_request->getPathInfo();
        if (strpos($arrArticleDetail['share_url'], rtrim($strLink, '/')) === FALSE)
        {
            $strLink = preg_replace('#/tin-tuc/sea-games/#', '/tuong-thuat/', $arrArticleDetail['share_url']);
            $strLink = $strLink . ($viewApp == 'app' ? '?view=app' : '');
            $this->_redirect($strLink, array('code' => 301));
        }
        ...
        {
            unset($arrDataExtend);
            $strLink = preg_replace('#/tuong-thuat/#', '/tong-thuat/', $arrArticleDetail['share_url']);
            //merge page extend to data detail
            $isReview = true;
        }
        ...
        $this->view->assign(array(
            ...
            'isReview' => $isReview,
            'strLinkVideo' => $strLinkVideo,
            'strLinkTongThuat' => $strLink . (empty($viewApp) ? '' : '?view=app'),
            'articleExtra' => $arrArticleExtra,
            'ogType' => 'website',
            'ogTitle' => $arrArticleDetail['title_format'] . ' - VnExpress SEA Games',
            'ogUrl' => preg_replace('#/tin-tuc/sea-games/#', '/tuong-thuat/', $arrArticleDetail['share_url']),
            'ogImage' => !empty($arrArticleDetail['thumbnail_url']) ? $this->view->ImageurlArticle($arrArticleDetail, 'size2') : 'http://m.f9.img.vnexpress.net/2014/01/10/fa68f81afe10b727852361b1894ccc74.jpg',
            'ogDescription' => trim(strip_tags(html_entity_decode($arrArticleDetail['lead'], ENT_COMPAT, 'UTF-8')))
        ));
    ...
    public function tongThuatAction()
    {
        ...
        $this->view->assign(array(
            ...
            'isArticleHotVnE' => $arrArticleDetail['privacy'] & 512,
            'timeUpdate' => $arrBlockDataId['time'],
            'strLinkTuongThuat' => '/tuong-thuat/' . $strLink . (empty($viewApp) ? '' : '?view=app'),
            'ogType' => 'website',
            'ogTitle' => $arrArticleDetail['title_format'] . ' - VnExpress SEA Games',
            'ogUrl' => preg_replace('#/tin-tuc/sea-games/#', '/tong-thuat/', $arrArticleDetail['share_url']),
            'ogImage' => !empty($arrArticleDetail['thumbnail_url']) ? $this->view->ImageurlArticle($arrArticleDetail, 'size2') : 'http://m.f9.img.vnexpress.net/2014/01/10/fa68f81afe10b727852361b1894ccc74.jpg',
            'ogDescription' => trim(strip_tags(html_entity_decode($arrArticleDetail['lead'], ENT_COMPAT, 'UTF-8')))
        ));
