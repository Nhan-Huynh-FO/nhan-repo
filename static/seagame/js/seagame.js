...
var thethao = {
    ...
    init : function() {
        ...
        if ($(".category_list_lich .scroll-pane").size() > 0) {
            $(".category_list_lich .scroll-pane").niceScroll(".category_list_lich .content_scoller",{touchbehavior:false,autohidemode:false});
        }

        /* Scroll top story */
        if ($(".list_hot_top .scroll-pane").size() > 0) {
            $(".list_hot_top .scroll-pane").niceScroll(".list_hot_top .content_scoller",{touchbehavior:false,autohidemode:false});
        }
        this.loadMore();
    },
    ...
    {
        var containerCarousel = $(container);
        //parser video carousel for home seagames
        var owl = containerCarousel.find('#owl-demo');
        owl.owlCarousel({
            autoPlay: 4000,
            items: 4,
            itemsDesktop: [1199, 3],
        ...
        containerCarousel.find('.next_slider').show().click(function () {
            owl.trigger('owl.next');
        });
        containerCarousel.find('.prev_slider').show().click(function () {
            owl.trigger('owl.prev');
        });
        containerCarousel.show();
    },
    loadMore: function() {
        ...
        $target.click(function (e) {
            ...
            if (status == 'unload') {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.error != 1) {
                            $(container).children('li:last').removeClass('last_item');
                            ...
                            $(container).children('li:last').addClass('last_item');
                        } else {
                            $target.parent().remove();
                        }
                    },
...
var sendmail = {
    ...
    tmp_sendmail: '<div id="login-vne5" class="login-form">'
        ...
        + '<ul>'
        + '<li class="active"><a href="' + base_url + '/" title="SEA Games">SEA Games</a></li>'
